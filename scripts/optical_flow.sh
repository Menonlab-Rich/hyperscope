...
PYTHON_SCRIPT="metavision_sparse_optical_flow.py"

# Define the list of accumulation times (-a parameter) to iterate over
ACCUMULATION_TIMES=(10 100 500 1000) # Bash array

# Other parameters for the python script
PARAM_STC_FILTER_THR="10000"
# --- End Configuration ---
...
echo "Accumulation Times: ${ACCUMULATION_TIMES[*]}" # Print the array elements
echo "=================================================="
...
find "$INPUT_DIR" -maxdepth 1 -name 'recording_2025-04-29*.raw' -print0 | while IFS= read -r -d $'\0' file_path; do
  ...
  if [ -f "$file_path" ]; then
    ...
    for param_a in "${ACCUMULATION_TIMES[@]}"; do

      # Construct the output filename including the accumulation time
      output_npy_filename="${OUTPUT_DIR}/optical_flow_${base_filename}_a${param_a}.npy"
      if [ -f "$output_npy_filename" ]; then
	      echo "$output_npy_filename exists. Skipping"
	      continue
      fi
      echo "  Running with Accumulation Time (a): $param_a"
      ...
      ../.venv/bin/python "$PYTHON_SCRIPT" \
        ...
        -a "$param_a" \
	--disable-stc \
        --output-sparse-npy-filename "$output_npy_filename" \
        --headless
