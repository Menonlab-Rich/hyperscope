...
class Threshold(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        scheduler_configs = { k: v for (k, v) in self.hparams.scheduler_configs.items() if k != "name" }
        optimizer_configs = { k: v for (k, v) in self.hparams.optimizer_configs.items() if k != "name" }
        ...
        if not Optimizer:
            warnings.warn(
                f"Optimizer {optimizer_name} not found! Fallback to Default: Adam"
            )
        ...
        if not Scheduler:
            warnings.warn(
                f"Scheduler: {scheduler_name} not found! Fallback to no scheduler"
            )
            return optimizer
