...
class ProjectionHead(nn.Module):
    """
    A simple projection head that takes the encoder's output and projects it
    into the desired space for the contrastive loss.
    This replaces the entire U-Net style Decoder.
    """
...
class Decoder(nn.Module):
    def __init__(self, in_shape, encoder_hidden_sizes, num_classes=1, bilinear=True):
        ...
        self.up1 = Up(
            encoder_hidden_sizes[3], encoder_hidden_sizes[2], encoder_hidden_sizes[2], bilinear
        )

        # Up2: Processes output from up1 and combines with encoder_fm[1]
        self.up2 = Up(
            encoder_hidden_sizes[2], encoder_hidden_sizes[1], encoder_hidden_sizes[1], bilinear
        )

        # Up3: Processes output from up2 and combines with encoder_fm[0]
        self.up3 = Up(
            encoder_hidden_sizes[1], encoder_hidden_sizes[0], encoder_hidden_sizes[0], bilinear
        )
    ...
    def forward(self, encoder_features):
        ...
        for i in range(len(downsample_factors)):
            ...
            if h_feat * w_feat != n:
                warnings.warn(
                    f"Feature map at stage {i} has {n} patches, which does not match the expected size of {h_feat}x{w_feat}. Reshaping based on 'n'."
                )
...
class Threshold(nn.Module):
    def __init__(
        ...
        # Encoder
        self.encoder = SwinForImageClassification.from_pretrained(
            encoder_name, ignore_mismatched_sizes=True
        )
        ...
        if decoder:
            ...
            base_hidden_size = (
                self.encoder.config.hidden_size // 8
            )  # This is the base channel dimension, e.g., 128
    ...
    def forward(self, x):
        ...
        if self.decoder:
            ...
            if logits.shape[2:] != self.out_shape:
                logits = F.interpolate(
                    logits, size=self.out_shape, mode="bilinear", align_corners=False
                )
        ...
        h = w = int(n**0.5)
        feature_map = last_hidden_state.permute(0, 2, 1).reshape(b, c, h, w)
        contrastive_features = self.projection_head(feature_map)

        dummy_logits = torch.zeros(b, 1, x.shape[2], x.shape[3], device=x.device)

        return dummy_logits, contrastive_features
...
if __name__ == "__main__":
    ...
    processor_256 = AutoImageProcessor.from_pretrained(
        "microsoft/swin-base-patch4-window7-224-in22k", size={"height": 256, "width": 256}
    )
