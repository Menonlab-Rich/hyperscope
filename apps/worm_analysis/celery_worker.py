...
def main(args):
    ...
    h5_file_path = Path(args.h5_file_path)
    ...
    try:
        with h5py.File(h5_file_path, "r") as hf:
            if "images" not in hf:
                logging.error("'images' dataset not found in H5 file.")
                sys.exit(1)
            ...
            for i in range(num_images):
                ...
                cropped_image_data[~full_mask] = 0
                cropped_image_data = (cropped_image_data - cropped_image_data.mean())/cropped_image_data.std()
                ...
                labeled_mask = (full_mask * args.label_value).astype(np.uint8)
...
if __name__ == "__main__":
    ...
    # --- File Arguments ---
    parser.add_argument("h5_file_path", type=str, help="Path to the input H5 file.")
