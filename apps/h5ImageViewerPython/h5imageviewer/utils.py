...
def find_regions_and_generate_mask(
    ...
    # Initial binary mask from Otsu or simple thresholding
    ...
    # Refinement using multi-Otsu if initial mask is noisy
    if mask.any():
        try:
            # Use multi-Otsu on the non-zero parts of the std dev image for better thresholds
            thresholds = threshold_multiotsu(std_dev_thresholded[std_dev_thresholded > 0])
            # Apply the upper threshold
            binary_img = (std_dev_thresholded >= thresholds[-1])
            # Fallback to lower threshold if the first result is too small
            ...
            # Reset mask and redraw with refined contours
            mask = np.zeros(output_mask_shape, dtype=np.uint8)
            for contour in contours:
                area = cv.contourArea(contour)
                if min_area <= area <= max_area:
            ...
            # Fallback if multi-Otsu fails (e.g., not enough distinct values)
            # The mask from the first pass will be used.
            pass


    # --- New K-Means Clustering Step ---
    if mask.any() and n_clusters > 1:
        ...
        if len(features) >= n_clusters:
            ...
            # Create a new mask for the clustered labels
            clustered_mask = np.zeros(output_mask_shape, dtype=np.uint8)
            # Assign labels back to the mask. Add 1 so labels are 1, 2, ..., k
            clustered_mask[pixels_to_cluster_coords] = kmeans.labels_ + 1
